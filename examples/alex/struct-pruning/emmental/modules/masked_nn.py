...
from .binarizer import  ThresholdBinarizer, TopKBinarizer, L1RowBinarizer, L1ColBinarizer, MagnitudeBinarizer

# TODO: add structured to this existing class by making it an option at init time
# Make the mask scores apply to the outputs rather than the weights. Scoring will be very different
class MaskedLinear(nn.Linear):
    ...
    def __init__(
        ...
        assert pruning_method in ["topK", "threshold", "sigmoied_threshold", "magnitude", "l0", "row", "col"]
    ...
    # computes binary mask given current scores, then does matrix mult with masked weights
    def forward(self, input: torch.tensor, threshold: float):
        ...
        elif self.pruning_method == "row":
            mask = L1RowBinarizer.apply(self.weight, threshold)
        elif self.pruning_method == "col":
            mask = L1ColBinarizer.apply(self.weight, threshold)
