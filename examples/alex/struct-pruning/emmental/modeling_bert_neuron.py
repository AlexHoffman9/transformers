...
from emmental import NeuronMaskedBertConfig
...
class BertLayer(nn.Module):
    ...
    # TODO here we could mask the intermediate output results
...
# changed this
@add_start_docstrings(
    """Neuron level masked Bert Model transformer with a sequence classification/regression head on top (a linear layer on top of
    the pooled output) e.g. for GLUE tasks. """,
    BERT_START_DOCSTRING,
)
class NeuronMaskedBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
